...
from torchrf.utils import insert_dims
from .awgn import AWGN

class ApplyTimeChannel(torch.nn.Module):
    # pylint: disable=line-too-long
    r"""ApplyTimeChannel(num_time_samples, l_tot, add_awgn=True, dtype=torch.complex64, **kwargs)
    ...
    x :  [batch size, num_tx, num_tx_ant, num_time_samples], torch.complex
        Channel inputs

    h_time : [batch size, num_rx, num_rx_ant, num_tx, num_tx_ant, num_time_samples + l_tot - 1, l_tot], torch.complex
        Channel responses.
        For each batch example, ``num_time_samples`` + ``l_tot`` - 1 time steps of a
        channel realization are required to filter the channel inputs.
    ...
    Output
    -------
    y : [batch size, num_rx, num_rx_ant, num_time_samples + l_tot - 1], torch.complex
        Channel outputs.
        The channel output consists of ``num_time_samples`` + ``l_tot`` - 1
        time samples, as it is the result of filtering the channel input of length
        ``num_time_samples`` with the time-variant channel filter  of length
    ...
    def call(self, inputs):
        ...
        x = torch.nn.functional.pad(x, [[0,0], [0,0], [0,0], [0,1]])
        x = insert_dims(x, 2, dim=1)

        x = torch.gather(x, self._g, dim=-1)
