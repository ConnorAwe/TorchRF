...
class ApplyOFDMChannel(torch.nn.Module):
    # pylint: disable=line-too-long
    r"""ApplyOFDMChannel(add_awgn=True, dtype=torch.complex64, **kwargs)
    ...
    x :  [batch size, num_tx, num_tx_ant, num_ofdm_symbols, fft_size], torch.complex
        Channel inputs

    h_freq : [batch size, num_rx, num_rx_ant, num_tx, num_tx_ant, num_ofdm_symbols, fft_size], torch.complex
    ...
    Output
    -------
    y : [batch size, num_rx, num_rx_ant, num_ofdm_symbols, fft_size], torch.complex
        Channel outputs
    """
