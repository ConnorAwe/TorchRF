...
from torchrf.utils import expand_to_rank

class BinaryMemorylessChannel(Layer):
    # pylint: disable=line-too-long
    r"""BinaryMemorylessChannel(return_llrs=False, bipolar_input=False, llr_max=100., dtype=torch.float32, **kwargs)
    ...
    def __init__(self, return_llrs=False, bipolar_input=False, llr_max=100.,dtype=torch.float32, **kwargs):
        ...
        self._temperature = tf.constant(0.1, torch.float32) # for Gumble-softmax
    ...
    def llr_max(self, value):
        ...
        self._llr_max = tf.cast(value, dtype=torch.float32)
    ...
    def temperature(self, value):
        ...
        self._temperature = tf.cast(value, dtype=torch.float32)
    ...
    def _custom_xor(self, a, b):
        ...
        if self.dtype in (torch.uint8, torch.uint16, torch.uint32, torch.uint64, torch.int8, torch.int16, torch.int32, torch.int64):
    ...
    def call(self, inputs):
        ...
        pb0 = tf.cast(pb0, torch.float32) # Gumble requires float dtypes
        pb1 = tf.cast(pb1, torch.float32) # Gumble requires float dtypes
        ...
        if self._return_llrs:
            ...
            # Remark: torchrf uses the logit definition log[p(x=1)/p(x=0)]
...
class BinarySymmetricChannel(BinaryMemorylessChannel):
    # pylint: disable=line-too-long
    r"""BinarySymmetricChannel(return_llrs=False, bipolar_input=False, llr_max=100., dtype=torch.float32, **kwargs)
...
class BinaryZChannel(BinaryMemorylessChannel):
    # pylint: disable=line-too-long
    r"""BinaryZChannel(return_llrs=False, bipolar_input=False, llr_max=100., dtype=torch.float32, **kwargs)
...
class BinaryErasureChannel(BinaryMemorylessChannel):
    # pylint: disable=line-too-long
    r"""BinaryErasureChannel(return_llrs=False, bipolar_input=False, llr_max=100., dtype=torch.float32, **kwargs)
    ...
    def call(self, inputs):
        ...
        # clip for numerical stability
        pb = tf.cast(pb, torch.float32) # Gumble requires float dtypes
        ...
        # if LLRs should be returned
        # remark: the torchrf logit definition is llr = log[p(x=1)/p(x=0)]
        if self._return_llrs:
            if not self._bipolar_input:
                x = 2 * x -1
