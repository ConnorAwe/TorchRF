...
from torchrf.rt.utils import cast
from . import scenes
...
class Scene:
    ...
    A scene is a collection of multiple instances of :class:`~torchrf.rt.SceneObject` which define
    the geometry and materials of the objects in the scene.
    The scene also includes transmitters (:class:`~torchrf.rt.Transmitter`) and receivers (:class:`~torchrf.rt.Receiver`)
    for which propagation :class:`~torchrf.rt.Paths`, channel impulse responses (CIRs) or coverage maps (:class:`~torchrf.rt.CoverageMap`) can be computed,
    as well as cameras (:class:`~torchrf.rt.Camera`) for rendering.

    The only way to instantiate a scene is by calling :meth:`~torchrf.rt.Scene,.load_scene`.
    ...
    def __init__(self, env_filename=None, dtype=torch.complex64):
    ...
    def cameras(self):
        """
        `dict` (read-only), { "name", :class:`~torchrf.rt.Camera`} : Dictionary
    ...
    def frequency(self, f):
        ...
        self._frequency = cast(f, self._rdtype)
        # Wavelength [m]
        self._wavelength = cast(SPEED_OF_LIGHT / f, self._rdtype)
    ...
    def dtype(self):
        r"""
        `torch.complex64 | torch.complex128` : Datatype used in tensors
        """
        return self._dtype
    ...
    def transmitters(self):
        # pylint: disable=line-too-long
        """
        `dict` (read-only), { "name", :class:`~torchrf.rt.Transmitter`} : Dictionary
    ...
    def receivers(self):
        """
        `dict` (read-only), { "name", :class:`~torchrf.rt.Receiver`} : Dictionary
    ...
    def radio_materials(self):
        # pylint: disable=line-too-long
        """
        `dict` (read-only), { "name", :class:`~torchrf.rt.RadioMaterial`} : Dictionary
            of radio materials
        """
    ...
    def objects(self):
        # pylint: disable=line-too-long
        """
        `dict` (read-only), { "name", :class:`~torchrf.rt.SceneObject`} : Dictionary
            of scene objects
        """
    ...
    def tx_array(self):
        """
        :class:`~torchrf.rt.AntennaArray` : Get/set the antenna array used by
    ...
    def size(self):
        """
        [3], torch.float : Get the size of the sceme, i.e., the size of the
        axis-aligned minimum bounding box for the scene
        """
        size = cast(self._scene.bbox().max - self._scene.bbox().min, self._rdtype)
        return size

    @property
    def center(self):
        """
        [3], torch.float : Get the center of the scene
        """
        size = cast((self._scene.bbox().max + self._scene.bbox().min) * 0.5, self._rdtype)
        return size
    ...
    def get(self, name):
        ...
        Output
        ------
        item : :class:`~torchrf.rt.SceneObject` | :class:`~torchrf.rt.RadioMaterial` | :class:`~torchrf.rt.Transmitter` | :class:`~torchrf.rt.Receiver` | :class:`~torchrf.rt.Camera` | `None`
    ...
    def add(self, item):
        ...
        Input
        ------
        item : :class:`~torchrf.rt.Transmitter` | :class:`~torchrf.rt.Receiver` | :class:`~torchrf.rt.RadioMaterial` | :class:`~torchrf.rt.Camera`
    ...
    def compute_paths(self, max_depth=3, method="fibonacci",
        ...
        (:attr:`~torchrf.rt.Scene.synthetic_array` is `True`), transmitters and receivers
        are modelled as if they had a single antenna located at their
        :attr:`~torchrf.rt.Transmitter.position`. The channel responses for each
        ...
        the help of the function :meth:`~torchrf.rt.Paths.apply_doppler` of the returned
        :class:`~torchrf.rt.Paths` object.
        ...
        .. code-block:: Python
            ...
            import torchrf
            from torchrf.rt import load_scene, Camera, Transmitter, Receiver, PlanarArray
        ...
        args = [max_depth, method, num_samples, los,
    ...
    def coverage_map(self,
        ...
        :attr:`~torchrf.rt.AntennaArray.positions`. Using a plane-wave assumption, the resulting phase shifts
        ...
        if ((cm_center is None)
            ...
            scene_min = cast(scene_min, self._rdtype)
            # In case of empty scene, bbox min is -inf
            scene_min = torch.where(torch.isinf(scene_min),
                                    -torch.ones_like(scene_min),
                                    scene_min)
            # [max_x, max_y, max_z]
            scene_max = self._scene.bbox().max
            scene_max = cast(scene_max, self._rdtype)
            # In case of empty scene, bbox min is inf
            scene_max = torch.where(torch.isinf(scene_max),
                                    torch.ones_like(scene_max),
                                    scene_max)
            cm_center = cast([(scene_min[0] + scene_max[0]) * 0.5,
                              (scene_min[1] + scene_max[1]) * 0.5,
                              1.5], self._rdtype)
            cm_size = cast([(scene_max[0] - scene_min[0]),
                            (scene_max[1] - scene_min[1])], self._rdtype)
            # Set the orientation to default value
            cm_orientation = torch.zeros([3], dtype=self._rdtype)
            ...
            cm_center = cast(cm_center, self._rdtype)
            cm_orientation = cast(cm_orientation, self._rdtype)
            cm_size = cast(cm_size, self._rdtype)

        # Check and initialize the combining and precoding vector
        if combining_vec is None:
            combining_vec = torch.ones(self.rx_array.num_ant, dtype=self._dtype)
            combining_vec /= torch.sqrt(cast(self.rx_array.num_ant, self._dtype))
        else:
            combining_vec = cast(combining_vec, self._dtype)
        if precoding_vec is None:
            num_tx = len(self.transmitters)
            precoding_vec = torch.ones(num_tx, self.tx_array.num_ant,
                                       dtype=self._dtype)
            precoding_vec /= torch.sqrt(cast(self.tx_array.num_ant, self._dtype))
        else:
            precoding_vec = cast(precoding_vec, self._dtype)
            precoding_vec = expand_to_rank(precoding_vec, 2, 0)

        # [3]
        rx_orientation = cast(rx_orientation, self._rdtype)
    ...
    def preview(self, paths=None, show_paths=True, show_devices=True,
        ...
        * Mouse right: Move
        ...
        Input
        -----
        paths : :class:`~torchrf.rt.Paths` | `None`
            Simulated paths generated by
            :meth:`~torchrf.rt.Scene.compute_paths()` or `None`.
        ...
        coverage_map : :class:`~torchrf.rt.CoverageMap` | `None`
    ...
    def render(self, camera, paths=None, show_paths=True, show_devices=True,
        ...
        Input
        ------
        camera : str | :class:`~torchrf.rt.Camera`
            The name or instance of a :class:`~torchrf.rt.Camera`.
            If an interactive viewer was opened with
            :meth:`~torchrf.rt.Scene.preview()`, set to `"preview"` to use its
            viewpoint.
    ...
    def preview_widget(self):
        """
        :class:`~torchrf.rt.InteractiveDisplay` : Get the preview widget
        """
...
def load_scene(filename=None, dtype=torch.complex64):
    # pylint: disable=line-too-long
    r"""
    ...
    -----
    filename : str
        Name of a valid scene file. torchrf uses the simple XML-based format
