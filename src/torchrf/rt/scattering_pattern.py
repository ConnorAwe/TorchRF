...
#
"""
Classes and functions related to Scattering patterns for the torchrf RT Module
"""
...
class ScatteringPattern(ABC):
    ...
    def __init__(self,
        ...
        self._lambda_ = torch.zeros(0, dtype=self._rdtype)
    ...
    def _update_params(alpha_max):
        ...
        j_even = torch.arange(0, alpha_max + 1, step=2, dtype=torch.float64)
        j_odd = torch.arange(1, alpha_max + 1, step=2, dtype=torch.float64)
        i_j_even = 2 * PI / (j_even + 1)
        n_max = (j_odd[-1] - 1) / 2
        w_range = torch.arange(0, n_max + 1, dtype=torch.float64)
        w_2 = 2 * w_range
        binom_2 = binom(w_2, w_range)
        binom_1 = torch.zeros([alpha_max + 1, alpha_max + 1])
        for alpha in range(alpha_max + 1):
            binom_1[alpha, :alpha + 1] = binom(alpha, torch.arange(alpha + 1)) / 2 ** alpha
        binom_1_even = binom_1[:, ::2].type(torch.float64)
        binom_1_odd = binom_1[:, 1::2].type(torch.float64)
        f_summands_even = torch.sum(binom_1_even * i_j_even, dim=-1)
        ...
        ScatteringPattern._params_32 = {
            "binom_1_odd": binom_1_odd.type(torch.float32),
            "binom_2": binom_2.type(torch.float32),
            "f_summands_even": f_summands_even.type(torch.float32),
            "j_odd": j_odd.type(torch.float32),
            "n_max": int(n_max),
            "w_2": w_2.type(torch.float32)
        }
    ...
    def f_alpha(cos_theta_i, alphas):
        ...
        cos_theta_i = cos_theta_i.unsqueeze(-1)
        ...
        n = torch.tensor(0, dtype=torch.int32)
        n_max = torch.tensor(params["n_max"], dtype=torch.int32)
        k_n = torch.Tensor([])

        while n <= n_max:
            k_n = torch.cat([k_n, series[..., :n + 1].sum(dim=-1).unsqueeze(0)])
            n += 1
        ...
        f_summands_odd = params["binom_1_odd"][alphas] * i_j_odd
        f_odd = torch.sum(f_summands_odd, dim=-1)

        # Get the precomputed sum over even values of j
        f_even = params["f_summands_even"][alphas]
    ...
    def pattern(k_i, k_s, n_hat, alpha_r, alpha_i, lambda_):
        ...
        k_r = k_i + 2 * torch.unsqueeze(cos_theta_i, dim=-1) * n_hat
        ...
        pattern_bs = lambda_ * torch.pow((1 + dot(k_r, k_s)) / 2,
                                         alpha_r.type(k_r.dtype))
        pattern_bs += (1 - lambda_) * torch.pow((1 - dot(k_i, k_s)) / 2,
                                                alpha_i.type(k_r.dtype))
        pattern_bs /= f

        # Compute Lambertian pattern
        pattern_l = cos_theta_s / PI
        pattern_l = torch.where(pattern_l < 0., 0, pattern_l)

        # Select one of the two models depending on alpha_r
        pattern = torch.where(alpha_r == 0, pattern_l, pattern_bs)
    ...
    def lambda_(self, value):
        ...
        try:
            self._lambda_ = value.type(self._rdtype)
        except AttributeError:
            self._lambda_ = torch.tensor(value, dtype=self._rdtype)
    ...
    def trainable_lambda_(self, value):
        ...
        self._lambda_.requires_grad_(value)
    ...
    def visualize(self, k_i=(0.7071, 0., -0.7071), show_directions=False):
        ...
        k_s = r_hat(theta_grid, phi_grid).type(self._rdtype)
        k_i = torch.broadcast_to(k_i_in, k_s.shape).type(self._rdtype)
        k_s = torch.reshape(k_s, [-1, 3])
        k_i = torch.reshape(k_i, [-1, 3])
        n_hat = torch.tensor([[0, 0, 1]], dtype=k_i.dtype).repeat(k_i.shape[0], 1)
        pattern = self(k_i, k_s, n_hat)
        pattern = torch.reshape(pattern, [50, 100])
        ...
        if show_directions:
            ...
            theta_r, phi_r = theta_i, phi_i + PI.type(phi_i.dtype)
            k_r = r_hat(theta_r, phi_r).numpy()
            plt.quiver(*[0, 0, 0], *k_r, length=r, linestyle='dashed', color="black", arrow_length_ratio=0.07,
                       alpha=0.5)
        ...
        ###
        k_i = torch.Tensor(k_i_in, self._rdtype)
        theta_i, phi_i = theta_phi_from_unit_vec(-k_i)
        theta_r, phi_r = theta_i, phi_i + PI.type(phi_i.dtype)

        # Pattern around reflected direction
        theta_s = torch.linspace(0.0, PI / 2, 100).type(self._rdtype)
        phi_s = torch.broadcast_to(phi_r, theta_s.shape)
        k_s = r_hat(theta_s, phi_s)
        k_i_1 = torch.broadcast_to(k_i, k_s.shape)
        n_hat = torch.Tensor([[0, 0, 1]], dtype=k_i.dtype)
        n_hat = torch.broadcast_to(n_hat, k_i_1.shape)
        ...
        if show_directions:
            ...
            ax.text(-theta_i - 10 * PI / 180, ax.get_yticks()[-1] * 2 / 3, r"$\hat{\mathbf{k}}_\mathrm{i}$",
                    horizontalalignment='center')
            ax.text(theta_i + 10 * PI / 180, ax.get_yticks()[-1] * 2 / 3, r"$\hat{\mathbf{k}}_\mathrm{r}$",
                    horizontalalignment='center')
