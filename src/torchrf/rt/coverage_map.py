...
from .utils import rotation_matrix, tf_matvec, cast, gather_nd
import warnings
...
class CoverageMap:
    ...
    :meth:`~torchrf.rt.Scene.coverage_map`. Please refer to the documentation of this function
    for further details.
    ...
    def __init__(self,
        ...
        if not isinstance(cell_size, torch.Tensor):
            cell_size = torch.tensor(cell_size)

        self._rdtype = eval(f'torch.{torch.finfo(dtype).dtype}')

        if (len(center.shape) != 1) or (center.size(0) != 3):
            msg = "`center` must be shaped as [x,y,z] (rank=1 and shape=[3])"
            raise ValueError(msg)

        if (len(orientation.shape) != 1) or (orientation.size(0) != 3):
            msg = "`orientation` must be shaped as [a,b,c]" \
                  " (rank=1 and shape=[3])"
            raise ValueError(msg)

        if (len(size.shape) != 1) or (size.size(0) != 2):
            msg = "`size` must be shaped as [w,h]" \
                  " (rank=1 and shape=[2])"
            raise ValueError(msg)

        if (len(cell_size.shape) != 1) or (cell_size.size(0) != 2):
            msg = "`cell_size` must be shaped as [w,h]" \
                  " (rank=1 and shape=[2])"
            raise ValueError(msg)

        num_cells_x = torch.ceil(size[0] / cell_size[0]).type(torch.int32)
        num_cells_y = torch.ceil(size[1] / cell_size[1]).type(torch.int32)

        if (len(value.shape) != 3) \
                or (value.size(1) != num_cells_y) \
                or (value.size(2) != num_cells_x):
            msg = "`value` must have shape" \
                  " [num_tx, num_cells_y, num_cells_x]"
            raise ValueError(msg)

        self._center = center.type(self._rdtype)
        self._orientation = orientation.type(self._rdtype)
        self._size = size.type(self._rdtype)
        self._cell_size = cell_size.type(self._rdtype)
        self._value = value.type(self._rdtype)
        ...
        ###############################################################
        # [num_cells_x]
        x_positions = torch.arange(num_cells_x, dtype=self._rdtype)
        x_positions = (x_positions + 0.5) * self._cell_size[0]
        # [num_cells_x, num_cells_y]
        x_positions = torch.unsqueeze(x_positions, dim=1)
        x_positions = torch.tile(x_positions, [1, num_cells_y])
        # [num_cells_y]
        y_positions = torch.arange(num_cells_y, dtype=self._rdtype)
        y_positions = (y_positions + 0.5) * self._cell_size[1]
        # [num_cells_x, num_cells_y]
        y_positions = torch.unsqueeze(y_positions, dim=0)
        y_positions = torch.tile(y_positions, [num_cells_x, 1])
        # [num_cells_x, num_cells_y, 2]
        cell_pos = torch.stack([x_positions, y_positions], dim=-1)
        # Move to global coordinate system
        # [1, 1, 2]
        size = expand_to_rank(self._size, len(cell_pos.shape), 0)
        # [num_cells_x, num_cells_y, 2]
        cell_pos = cell_pos - size * 0.5
        # [num_cells_x, num_cells_y, 3]
        cell_pos = torch.concat([cell_pos,
                                 torch.zeros([num_cells_x, num_cells_y, 1],
                                             dtype=self._rdtype)],
                                dim=-1)
        # [3, 3]
        rot_cm_2_gcs = rotation_matrix(self._orientation)
        # [1, 1, 3, 3]
        rot_cm_2_gcs_ = expand_to_rank(rot_cm_2_gcs, len(cell_pos.shape) + 1,
                                       axis=0)
        # [num_cells_x, num_cells_y, 3]
        cell_pos = tf_matvec(rot_cm_2_gcs_, cell_pos)
        # [num_cells_x, num_cells_y, 3]
        cell_pos = cell_pos + self._center
        # [num_cells_y, num_cells_x, 3]
        cell_pos = torch.permute(cell_pos, [1, 0, 2])
        ...
        # [num_tx, 3]
        center_ = torch.unsqueeze(self._center, dim=0)
        tx_pos = tx_pos - center_
        # [3, 3]
        rot_gcs_2_cm = rot_cm_2_gcs.T
        # [1, 3, 3]
        rot_gcs_2_cm_ = torch.unsqueeze(rot_gcs_2_cm, dim=0)
        # Transmitter positions in the coverage map system
        # [num_tx, 3]
        tx_pos = tf_matvec(rot_gcs_2_cm_, tx_pos)
        # Keep only x and y
        # [num_tx, 2]
        tx_pos = tx_pos[:, :2]
        ...
        # Quantizing
        # [num_tx, 2]
        tx_pos = torch.floor(tx_pos / self._cell_size).type(torch.int32)
        self._tx_pos = tx_pos

    @property
    def center(self):
        """
        [3], torch.float : Get the center of the coverage map
        """
        return self._center
    ...
    def orientation(self):
        """
        [3], torch.float : Get the orientation of the coverage map
        """
    ...
    def size(self):
        """
        [2], torch.float : Get the size of the coverage map
        """
        return self._size
    ...
    def cell_size(self):
        """
        [2], torch.float : Get the resolution of the coverage map, i.e., width
    ...
    def cell_centers(self):
        """
        [num_cells_y, num_cells_x, 3], torch.float : Get the positions of the
    ...
    def as_tensor(self):
        ...
        Output
        ------
        : [num_tx, num_cells_y, num_cells_x], torch.float
    ...
    def sample_positions(self, batch_size, tx=0, min_gain_db=None,
        ...
        min_gain_db = torch.tensor(min_gain_db, dtype=self._rdtype)

        if max_gain_db is None:
            max_gain_db = np.infty
        max_gain_db = torch.tensor(max_gain_db, dtype=self._rdtype)
        ...
        if min_dist is None:
            min_dist = 0.
        min_dist = torch.tensor(min_dist, dtype=self._rdtype)

        if max_dist is None:
            max_dist = np.infty
        max_dist = torch.tensor(max_dist, dtype=self._rdtype)
        ...
        cm_db = 10. * torch.log10(self._value[tx, :, :])

        # Set min and max distance
        tx_pos = torch.reshape(tx_pos, (1, 1, 3)).type(self._rdtype)
        d = torch.norm(cell_centers - tx_pos, dim=2)
        cm_inf = torch.full(cm_db.shape, fill_value=-1. * np.infty, dtype=self._rdtype)
        cm_db = torch.where(d < min_dist, cm_inf, cm_db)  # min dist
        cm_db = torch.where(d > max_dist, cm_inf, cm_db)  # max dist

        # Get all indices of positions with large enough path_gain
        idx = torch.nonzero(torch.logical_and(cm_db > min_gain_db,
                                            cm_db < max_gain_db), as_tuple=False)

        # Duplicate indices if requested batch_size > num_idx
        reps = torch.ceil(cast(batch_size, torch.int32) / idx.shape[0])
        reps = torch.unsqueeze(reps, dim=0).type(torch.int32)
        reps = torch.concat((reps, torch.ones_like(cast(idx.shape[1:], dtype=torch.int32))),
                            dim=0)
        idx = torch.tile(idx, tuple(reps))  # and repeat positions

        # Randomly permute indices
        id = torch.randperm(idx.size(0))
        idx = idx[id, :]
        # idx = tf.random.shuffle(idx) # Original tf implementation.

        # Sample batch_size random positions
        ue_pos = gather_nd(self.cell_centers, idx[:batch_size])

        # Add random offset within cell-size, if positions should not be
        # centered
        if not center_pos:
            # cell can be rotated
            dir_x = torch.unsqueeze(0.5 * (cell_centers[0, 0] - cell_centers[1, 0]), dim=0)
            dir_y = torch.unsqueeze(0.5 * (cell_centers[0, 0] - cell_centers[0, 1]), dim=0)

            rand_x = 2.0 * torch.rand(batch_size, 1, dtype=self._rdtype) - 1.0
            rand_y = 2.0 * torch.rand(batch_size, 1, dtype=self._rdtype) - 1.0
...
def coverage_map_rectangle_to_world(center, orientation, size):
    ...
    Input
    ------
    center : [3], torch.float
        Center of the rectangle

    orientation : [3], torch.float
