...
from .utils import normalize, dot, theta_phi_from_unit_vec, cross, \
    mi_to_torch_tensor, scatter_nd_update
from torchrf.constants import PI


class SolverBase:
    # pylint: disable=line-too-long
    r"""SolverBase(scene, solver=None, dtype=torch.complex64)
    ...
    def __init__(self, scene, solver=None, dtype=torch.complex64):
        ...
        # [num_objects]
        prim_offsets = torch.tensor(prim_offsets, dtype=torch.int32)
        primitives_2_objects = torch.tensor(primitives_2_objects, dtype=torch.int32)

        # Tensor of triangles vertices
        # [n_prims, number of vertices : 3, coordinates : 3]
        prims = torch.zeros(n_prims, 3, 3, dtype=self._rdtype)
        # Normals to the triangles
        normals = torch.zeros(n_prims, 3, dtype=self._rdtype)
        ...
        for prim_offset, s in zip(prim_offsets, mi_scene.shapes()):
            ...
            # Move to TensorFlow
            # [n_prims*3, 3]
            vertex_coords = mi_to_torch_tensor(vertex_coords, self._rdtype)
            # Unflatten
            # [n_prims, vertices per triangle : 3, 3]
            vertex_coords = torch.reshape(vertex_coords, [s.face_count(), 3, 3])
            # Update the `prims` tensor
            sl = torch.arange(prim_offset, prim_offset + s.face_count(),
                              dtype=torch.int32)
            sl = torch.unsqueeze(sl, dim=1)
            prims = scatter_nd_update(prims, sl, vertex_coords)
            ...
            # Move to TensorFlow
            # [n_prims, 3]
            n = mi_to_torch_tensor(mi_n, self._rdtype)
            # Update the 'normals' tensor
            normals = scatter_nd_update(normals, sl, n)
    ...
    def _build_scene_object_properties_tensors(self):
        ...
        relative_permittivity = torch.zeros(num_shapes, dtype=self._dtype)
        scattering_coefficient = torch.zeros(num_shapes, dtype=self._rdtype)
        xpd_coefficient = torch.zeros(num_shapes, dtype=self._rdtype)
        alpha_r = torch.zeros(num_shapes, dtype=torch.int32)
        alpha_i = torch.zeros(num_shapes, dtype=torch.int32)
        lambda_ = torch.zeros(num_shapes, dtype=self._rdtype)
        for rm in self._scene.radio_materials.values():
            using_objects = torch.tensor(rm.using_objects, dtype=torch.int32)
            num_using_objects = using_objects.size()[0]
            if num_using_objects == 0:
                continue

            relative_permittivity = scatter_nd_update(relative_permittivity,
                                                      torch.reshape(using_objects, (-1, 1)),
                                                      torch.full([num_using_objects],
                                                                 rm.complex_relative_permittivity))

            scattering_coefficient = scatter_nd_update(scattering_coefficient,
                                                       torch.reshape(using_objects, [-1, 1]),
                                                       torch.full([num_using_objects], rm.scattering_coefficient))

            xpd_coefficient = scatter_nd_update(xpd_coefficient,
                                                torch.reshape(using_objects, [-1,1]),
                                                torch.full([num_using_objects], rm.xpd_coefficient))

            alpha_r = scatter_nd_update(alpha_r,
                                        torch.reshape(using_objects, [-1, 1]),
                                        torch.full([num_using_objects], rm.scattering_pattern.alpha_r))

            alpha_i = scatter_nd_update(alpha_i,
                                        torch.reshape(using_objects, [-1, 1]),
                                        torch.full([num_using_objects], rm.scattering_pattern.alpha_i))

            lambda_ = scatter_nd_update( lambda_,
                                         torch.reshape(using_objects, [-1,1]),
                                         torch.full([num_using_objects], rm.scattering_pattern.lambda_))
    ...
    def _test_obstruction(self, o, d, maxt):
        ...
        Input
        -----
        o: [batch_size, 3], torch.float
            Origin of the rays

        d: [batch_size, 3], torch.float
            Direction of the rays.
            Must be unit vectors.

        maxt: [batch_size], torch.float
            Length of the ray.

        Output
        -------
        val: [batch_size], torch.bool
        ...
        val = mi_to_torch_tensor(mi_val, torch.bool)
        return val
    ...
    def _extract_wedges(self):
        ...
        # TODO: when is the right time to convert these floats to tensors?  Probably best when they are initially set
        #  in SolverBase?
        angle_threshold = torch.tensor(SolverBase.WEDGES_ANGLE_THRESHOLD, dtype=self._rdtype)
        ...
        # List all edges
        # [num_prim, 2 + 2 + 2, 3]
        all_edges_undirected = torch.cat([v0, v1, v1, v2, v2, v0], dim=1)

        # [num_edges = num_prim*3, 2, 3]
        all_edges_undirected = torch.reshape(all_edges_undirected,
        ...
        # [num_prim, 3, 3]
        remaining_vertex = torch.cat([v2, v0, v1], dim=1)

        # [num_edges, 3]
        remaining_vertex = torch.reshape(remaining_vertex, [-1, 3])
        ...
        unique_edges, indices_of_unique = torch.unique(all_edges, dim=0, return_inverse=True)

        # Number of occurences of every unique edge
        # [num_unique_edges]
        # Note that torch_unique sorts its output, where tf.unique is ordered like original list.
        _, unique_indices_count = torch.unique(indices_of_unique, return_counts=True)

        # Flag indicating which edges shared by exactly one or two primitives,
        # i.e., edges or wedges
        # [num_unique_edges]
        is_selected = torch.logical_or(torch.eq(unique_indices_count, 1),
                                       torch.eq(unique_indices_count, 2))
        ...
        # [num_edges]
        seq = torch.arange(indices_of_unique.shape[0], dtype=torch.int32)
        # [num_unique_edges]
        default = -torch.ones(unique_edges.shape[0], dtype=torch.int32)
        # [num_unique_edges]
        all_edges_index_1 = scatter_nd_update(default, indices_of_unique[:, None], seq)

        # Next, list the second primitive that the wedge is connected to.
        # -1 is used for edges defined by a single primitive (screens)
        # [num_unique_edges]
        false_value = torch.zeros(all_edges_index_1.shape[0], dtype=torch.bool)
        # [num_edges]
        missing = torch.ones(indices_of_unique.shape[0], dtype=torch.bool)
        missing = scatter_nd_update(missing, all_edges_index_1[:, None], false_value)
        # [num_unique_edges]
        # TODO: many of these tensors used to have a second dimension that I've been getting rid of
        all_edges_index_2 = scatter_nd_update(default, indices_of_unique[missing][:, None], seq[missing])
        ...
        # [num_unique_edges]
        is_edge = torch.eq(all_edges_index_2, -1)
        ...
        # [num_unique_edges]
        all_edges_index_2 = torch.where(is_edge, all_edges_index_1, all_edges_index_2)
        ...
        # [num_unique_edges]
        flip = torch.where(torch.gt(cos_angle, torch.zeros_like(cos_angle)),
                           -torch.ones_like(cos_angle),
                           torch.ones_like(cos_angle))
        # [num_unique_edges, 1]
        flip = torch.unsqueeze(flip, dim=1)
        # [num_unique_edges, 3]
        n1 = n1 * flip
        n2 = n2 * flip
        ...
        # ()
        theshold = torch.abs(torch.sin(angle_threshold))
        # [num_unique_edges]
        is_selected_ = torch.greater(torch.abs(cos_angle), theshold)
        # Don't discard edges
        # [num_unique_edges]
        is_selected_ = torch.logical_or(is_edge, is_selected_)
        # [num_unique_edges]
        is_selected = torch.logical_and(is_selected, is_selected_)
        ...
        # n1: 0-face
        # n2: n-face
        normals = torch.stack([n1, n2], dim=1)
        ...
        seq = torch.arange(selected_edges.shape[0], dtype=torch.int32)
        unique_edge_index_to_double_edge_index = scatter_nd_update(default, selected_indices[:, None], seq)
        # [num_prim, 3]
        prim_to_wedges = torch.reshape(unique_edge_index_to_double_edge_index[indices_of_unique], (-1, 3))

        # Indices of the objects to which each edge belongs
        # First, indices (in all_edges) of edges
        # [num_unique_edges, 2]
        wedges_indices = torch.stack([all_edges_index_1, all_edges_index_2], dim=1)
        ...
        # [num_selected_edges, 2]
        wedges_2_object = self._primitives_2_objects[wedges_2_prim]
        ...
        # [num_selected_edges, 3]
        origin_indicator = torch.unsqueeze(origin_indicator, dim=1)
        origin = torch.where(origin_indicator < 0,
                             selected_wedges_end,
                             selected_wedges_start)
        # Set arbitrarely the vector for the edges
        # [num_selected_edges, 3]
        e_hat = torch.where(torch.unsqueeze(is_edge, dim=1), e_hat_ind, e_hat)
    ...
    def _swap_edges(self, edges):
        ...
        needs_swap_1 = r0 > r1
        not_disc_1 = torch.isclose(r0, r1)
        needs_swap_2 = torch.logical_and(not_disc_1, phi0 > phi1)
        not_disc_2 = torch.isclose(phi0, phi1)
        not_disc_12 = torch.logical_and(not_disc_1, not_disc_2)
        needs_swap_3 = torch.logical_and(not_disc_12, theta0 > theta1)
        needs_swap = torch.any(torch.stack([needs_swap_1,
                                            needs_swap_2,
                                            needs_swap_3], dim=1),
                               dim=1, keepdim=True)

        result = torch.cat([
            torch.unsqueeze(torch.where(needs_swap, p1, p0), dim=1),
            torch.unsqueeze(torch.where(needs_swap, p0, p1), dim=1),
        ], dim=1)
        return result
    ...
    def _wedges_from_primitives(self, candidates, edge_diffraction):
        ...
        if candidates.shape[0] == 0:
            return torch.Tensor([], torch.int32)
        ...
        # [num_samples, 3]
        candidate_wedges = self._primitives_2_wedges[candidates]
        # [num_samples*3]
        candidate_wedges = torch.reshape(candidate_wedges, [-1])

        # Remove -1
        # [<= num_samples*3]
        candidate_wedges = candidate_wedges[candidate_wedges != -1]

        # Remove duplicates
        # [num_candidate_wedges]
        candidate_wedges = torch.unique(candidate_wedges)

        # Remove edges if required
        if not edge_diffraction:
            # [num_candidate_wedges]
            is_wedge = ~self._is_edge[candidate_wedges]
            wedge_indices = torch.nonzero(is_wedge, as_tuple=False)[:, 0]
            # [num_candidate_wedges]
            candidate_wedges = candidate_wedges[wedge_indices]
